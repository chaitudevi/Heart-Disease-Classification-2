import pandas as pd
import numpy as np
from src.data.preprocess import clean_data, impute_missing, encode_categorical, preprocess_pipeline
...
def test_preprocess_pipeline_runs():
    ...
    """
    df = pd.DataFrame({"age":[60,55], "sex":[1,0], "cp":[3,2], "trestbps":[120,130],
                       "chol":[240,250], "fbs":[0,1], "restecg":[1,0], "thalach":[150,160],
                       "exang":[0,1], "oldpeak":[2.3,1.5], "slope":[2,1], "ca":[0,1],
                       "thal":[2,3], "target":[1,0]})
    processed = preprocess_pipeline(df, ["sex","cp","fbs","restecg","exang","slope","thal","ca"],
